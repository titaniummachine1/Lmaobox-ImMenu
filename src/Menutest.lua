...
local function advancedElements()
    ...
    local selectedOption = options[menuState.selectedOption]
    ImMenu.Text("Selected option: " .. selectedOption)

    -- Change color based on the selected option
    if selectedOption == "Red" then
    ...
    local weaponOptions = {"Melee", "Primary", "Secondary"}
    -- Display a combo box to choose weapons (multiple selection)
    menuState.weaponSelections = ImMenu.Combo("Choose Weapons", menuState.weaponSelections or {}, weaponOptions)

    -- Display selected weapons from combo box
    local selectedWeapons = {}
    for i, selected in ipairs(menuState.weaponSelections) do
        if selected then
            table.insert(selectedWeapons, weaponOptions[i])
        end
    end
    ImMenu.Text("Selected weapons: " .. table.concat(selectedWeapons, ", "))
